...
class PFLocaliser(PFLocaliserBase):
    ...
    def initialise_particle_cloud(self, initialpose):
        ...
        for _ in range(num_particles):
            ...
            particle.position.x = initialpose.pose.pose.position.x + gauss(0, 0.9)  # Adjust noise parameters as needed
            particle.position.y = initialpose.pose.pose.position.y + gauss(0, 0.9)  # Adjust noise parameters as needed

            initial_yaw = getHeading(initialpose.pose.pose.orientation)
            particle.orientation = rotateQuaternion(initialpose.pose.pose.orientation, gauss(0, 0.05))  # Adjust noise parameters as needed
